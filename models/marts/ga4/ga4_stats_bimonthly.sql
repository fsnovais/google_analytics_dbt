...
with
    stats_daily as (
        select *, date_trunc(date, isoweek) as week_date from {{ ref("ga_joined") }}
    ),
    stats_monthly_pre as (
        select
            ...
            hostname,
            case
                when cast(mod(extract(week from week_date), 2) as bool)
                then
                    date_trunc(date_add(week_date, interval 1 week), isoweek)
                    + interval 6 day
            ...
            -- end as week_number
            case
                when cast(mod(extract(isoweek from week_date), 2) as bool)
