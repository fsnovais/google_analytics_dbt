select
    ...
    max(engaged_sessions) engaged_sessions,
    max(engagement_time_seconds) engagement_time_seconds,
    max(rank) rank,
        ...
        from
            {{
                dbt_utils.union_relations(
                    [
                        ref("ga_gsc_organic_join_keyword"),
                        ref("accuranker_keyword_mappings"),
                    ]
                )
            }}
    )
