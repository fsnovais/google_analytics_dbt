...
from
    (
        select
            date_trunc(
                parse_date("%m/%d/%Y", split(timestamp, " ")[offset(0)]), month
            ) month_date,
                        ...
                        regexp_replace(
                            replace(
                                replace(
                                    replace(
                                        replace(
                                            preferred_landing_page_path, 'www.', ''
                                        ),
                                        'http://',
                                        ''
                                    ),
            ...
            competitor_rank,
            regexp_extract(
                competitor_highest_ranking_page, r'^(?:https?:\/\/)?(?:www\.)?([^\/]+)'
                        ...
                        regexp_replace(
                            replace(
                                replace(
                                    replace(
                                        replace(
                                            competitor_highest_ranking_page, 'www.', ''
                                        ),
                                        'http://',
                                        ''
                                    ),
