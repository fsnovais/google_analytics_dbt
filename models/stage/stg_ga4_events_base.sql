...
with
    ...
    domain_tenants as (
        select distinct
            site_name,
            string_agg(
                distinct split(primary_domain, '/')[safe_ordinal(2)], ', '
    ...
    unnested as (
        select
            ...
            split(trim(split(_dbt_source_relation, '.')[safe_ordinal(2)], '`'), '_')[
                safe_ordinal(2)
            ] as ga4_dataset_id,
            ...
            case
                when event_name = 'affiliate_link_click' then 1 else 0
    ...
    final as (
        select
            ...
            to_base64(
                md5(concat(stream_id, user_pseudo_id, cast(ga_session_id as string)))
            ) as session_key,
            to_base64(
                md5(
                    (
                        concat(
                            cast(event_date_dt as string),
                            cast(
                                extract(
                                    hour from timestamp_micros(event_timestamp)
                                ) as string
                            ),
                            page_location
                        )
            ...
            lower(
                regexp_replace(
                    replace(
                        replace(replace(page_location, 'www.', ''), 'http://', ''),
                        'https://',
                        ''
                    ),
                    r'\#.*$',
        ...
        left outer join
            domain_tenants on domain_tenants.site_name = dataset_metadata.site_name

        window
            session_window as (
                partition by
                    concat(stream_id, user_pseudo_id, cast(ga_session_id as string))
            )
